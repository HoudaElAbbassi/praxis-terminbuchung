                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredAppointments.map((appointment, index) => (
                    <tr
                      ...
                      className={`hover:bg-[#e8f4f2] transition-colors cursor-pointer ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                    >
                      {/* Datum */}
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between p-5 border-b border-gray-200">
              <div>
                <h3 className="text-xl font-bold text-[#2c5f7c]" style={{fontFamily: "'Playfair Display', serif"}}>
                  {detailAppointment.user.firstName} {detailAppointment.user.lastName}
                </h3>
                <div className="flex items-center gap-2 mt-1">
                  <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${
                    detailAppointment.status === "PENDING" ? "bg-yellow-100 text-yellow-800" :
                    detailAppointment.status === "PROPOSAL_SENT" ? "bg-purple-100 text-purple-800" :
                    detailAppointment.status === "CONFIRMED" ? "bg-green-100 text-green-800" :
                    detailAppointment.status === "CANCELLED" ? "bg-red-100 text-red-800" :
                    detailAppointment.status === "COMPLETED" ? "bg-blue-100 text-blue-800" :
                    "bg-gray-100 text-gray-800"
                  }`}>
                    {detailAppointment.status === "PENDING" ? "⏳ Ausstehend" :
                     detailAppointment.status === "PROPOSAL_SENT" ? "📧 Vorschlag gesendet" :
                     detailAppointment.status === "CONFIRMED" ? "✓ Bestätigt" :
                     detailAppointment.status === "CANCELLED" ? "✗ Abgesagt" :
                     detailAppointment.status === "COMPLETED" ? "✓ Abgeschlossen" :
                     detailAppointment.status}
                  </span>
                  <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${
                    detailAppointment.urgency === 'URGENT' ? 'bg-red-100 text-red-700' :
                    detailAppointment.urgency === 'FLEXIBLE' ? 'bg-green-100 text-green-700' :
                    'bg-yellow-100 text-yellow-700'
                  }`}>
                    {detailAppointment.urgency === 'URGENT' ? '🔴 Dringend' :
                     detailAppointment.urgency === 'FLEXIBLE' ? '🟢 Flexibel' : '🟡 Normal'}
                  </span>
                </div>
              </div>
              <button
            ...
            <div className="p-5 space-y-4">
              {/* Termin-Zeit */}
              <div className="bg-[#e8f4f2] rounded-lg p-3">
                {detailAppointment.date ? (
                  <div className="flex items-center gap-2 text-[#2c5f7c] font-semibold">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                    {formatDate(detailAppointment.date)}
                    {detailAppointment.startTime && (
                      <span>· {formatTime(detailAppointment.startTime)}{detailAppointment.endTime ? ` – ${formatTime(detailAppointment.endTime)}` : ""}</span>
                    )}
                  </div>
                ) : (
                  <div className="space-y-1">
                    <p className="text-xs font-semibold text-gray-600">Terminpräferenzen</p>
                    <p className="text-sm text-[#2c5f7c]">📅 {translateDays(detailAppointment.preferredDays)}</p>
                    <p className="text-sm text-[#4a9d8f]">🕐 {translateTimeSlots(detailAppointment.preferredTimeSlots)}</p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <p className="flex items-center gap-2 text-gray-700 break-all">
                  ...
                  {detailAppointment.user.email}
                </p>
                {detailAppointment.user.dateOfBirth && (
                  <p className="flex items-center gap-2 text-gray-700">
                    <svg className="w-4 h-4 text-[#4a9d8f] flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd" />
                    </svg>
                    Geb. {new Date(detailAppointment.user.dateOfBirth).toLocaleDateString("de-DE")}
                  </p>
                )}
                {detailAppointment.user.address && (
                  <p className="flex items-center gap-2 text-gray-700">
                    <svg className="w-4 h-4 text-[#4a9d8f] flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
              ...
              {/* Termin-Details */}
              <div className="space-y-2 text-sm">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Termin-Details</p>
                <p className="flex items-center gap-2 text-gray-700">
                  <svg className="w-4 h-4 text-[#4a9d8f] flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd" />
                  </svg>
                  <span className="font-medium">Terminart:</span> {detailAppointment.appointmentType.name} ({detailAppointment.appointmentType.duration} Min.)
                </p>
                <p className="flex items-center gap-2 text-gray-700">
                  <svg className="w-4 h-4 text-[#4a9d8f] flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.465 5.05l-.708-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 000 2h1z" clipRule="evenodd" />
                  </svg>
                  <span className="font-medium">Versicherung:</span> {translateInsuranceType(detailAppointment.insuranceType)}
                </p>
                <p className="flex items-center gap-2 text-gray-700">
                  <svg className="w-4 h-4 text-[#4a9d8f] flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z" />
                  ...
                  <span className="font-medium">Art:</span> {detailAppointment.isFirstVisit ? '🆕 Ersttermin' : '🔄 Folgetermin'}
                </p>
                {detailAppointment.reasonForVisit && (
                  <p className="flex items-start gap-2 text-gray-700">
                    <svg className="w-4 h-4 text-[#4a9d8f] flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" />
                    </svg>
                    <span><span className="font-medium">Grund:</span> {detailAppointment.reasonForVisit}</span>
                  </p>
                )}
                {detailAppointment.notes && (
                  <p className="flex items-start gap-2 text-gray-700">
                    ...
                    <span><span className="font-medium">Notizen:</span> {detailAppointment.notes}</span>
                  </p>
                )}
                {detailAppointment.specialRemarks && (
                  <div className="bg-purple-50 border-l-4 border-purple-400 p-3 rounded mt-2">
                    <p className="text-xs font-semibold text-purple-800 mb-1">Besondere Anmerkungen</p>
                    <p className="text-sm text-gray-700">{detailAppointment.specialRemarks}</p>
                  </div>
                )}
              </div>
              ...
              <button
                onClick={() => {
                  toggleHandledInternally(detailAppointment.id, detailAppointment.handledInternally);
                  setDetailAppointment({ ...detailAppointment, handledInternally: !detailAppointment.handledInternally });
                }}
                className={`w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
                  detailAppointment.handledInternally
                    ? 'bg-teal-100 text-teal-800 hover:bg-teal-200 border border-teal-300'
                    : 'bg-gray-100 text-gray-600 hover:bg-gray-200 border border-gray-200'
                }`}
              >
                {detailAppointment.handledInternally ? (
                  <>
                    ...
                    Intern erledigt – Klicken zum Zurücksetzen
                  </>
                ) : (
                  <>
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="p-5 border-t border-gray-200 space-y-2">
              {detailAppointment.status === "PENDING" && (
                <>
                  <div className="flex gap-2">
                    <button
                      onClick={() => { setDetailAppointment(null); handleSendProposal(detailAppointment); }}
                      className="flex-1 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2.5 rounded-lg font-semibold text-sm transition-colors"
                    >
                      📧 Vorschlag senden
                    </button>
                    <button
                      onClick={() => { setDetailAppointment(null); handleSetAppointmentTime(detailAppointment); }}
                      className="flex-1 bg-[#4a9d8f] hover:bg-[#3d8378] text-white px-4 py-2.5 rounded-lg font-semibold text-sm transition-colors"
                    >
                      📅 Direkt festlegen
                    </button>
                  </div>
                  <button
                    onClick={() => { setDetailAppointment(null); openConfirmDialog(detailAppointment.id, "CANCELLED"); }}
                    className="w-full bg-red-600 hover:bg-red-700 text-white px-4 py-2.5 rounded-lg font-semibold text-sm transition-colors"
                  >
                    Ablehnen
                  </button>
                </>
              ...
              {detailAppointment.status === "CONFIRMED" && (
                <div className="flex gap-2">
                  <button
                    onClick={() => { setDetailAppointment(null); openConfirmDialog(detailAppointment.id, "COMPLETED"); }}
                    className="flex-1 bg-[#2c5f7c] hover:bg-[#1f4459] text-white px-4 py-2.5 rounded-lg font-semibold text-sm transition-colors"
                  >
                    Als abgeschlossen markieren
                  </button>
                  <button
              ...
              {detailAppointment.status === "PROPOSAL_SENT" && (
                <button
                  onClick={() => { setDetailAppointment(null); openConfirmDialog(detailAppointment.id, "CANCELLED"); }}
                  className="w-full bg-red-600 hover:bg-red-700 text-white px-4 py-2.5 rounded-lg font-semibold text-sm transition-colors"
                >
              ...
              <button
                onClick={() => setDetailAppointment(null)}
                className="w-full px-4 py-2.5 border-2 border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50 font-semibold text-sm transition-colors"
              >
                Schließen
              </button>
            </div>
